...
const QrCodesPage = () => {
  ...
  const getUpiString = (amount = 0) => {
    ...
    let rawVal = activeQr.upiId || "";
    
    // Remove whitespace
    rawVal = rawVal.trim();

    // 1. If it's an unverified/placeholder QR, we shouldn't allow generating dynamic amount because it's fake.
    if (rawVal.startsWith('MANUAL-UPI')) {
        return rawVal; // Just return it, the UI will warn them.
    }

    // 2. If it's BHARAT-QR / EMV Co (Starts with 000201...)
    if (rawVal.startsWith('000201')) {
        // Appending &am= to EMV Co string breaks the whole TLV structure.
        // It's technically impossible to safely inject dynamic amount without full TLV parser.
        // So we strictly return original.
        return rawVal;
    }

    // Prepare components
    // VPAs strictly require the '@' to be un-encoded for many UPI apps to recognize it
    const pn = encodeURIComponent(activeQr.merchantName || user?.name || 'Merchant');
    const mc = '5499'; // Valid Retail MCC
    const tid = activeQr.tid || '';
    // UUIDs are 36 chars, but TR max length in some UPI specs is 35. Slice to safe length.
    ...
    // 3. If it's already a full UPI URI
    if (rawVal.startsWith('upi://') || rawVal.startsWith('UPI://')) {
        let uri = rawVal;
        
        // If dynamic amount is requested, we need to inject it
        if (amount > 0) {
            // Safely inject or replace 'am'
            if (uri.match(/[?&]am=/i)) {
                uri = uri.replace(/([?&])am=[0-9.]+/i, `$1am=${reqAmount}`);
            } else {
                uri += `&am=${reqAmount}`;
            }
            
            // Safely inject or replace 'mam' 
            if (uri.match(/[?&]mam=/i)) {
                uri = uri.replace(/([?&])mam=[0-9.]+/i, `$1mam=${reqAmount}`);
            } else {
                uri += `&mam=${reqAmount}`;
            }
            
            // Standardize currency if missing
            if (!uri.match(/[?&]cu=/i)) uri += `&cu=INR`;
        }
        return uri;
    }
    
    // 4. Otherwise, assume it's just a raw VPA (e.g., 919955@paytm)
    // Do not encodeURIComponent because it turns '@' into '%40' which breaks scanners!
    const pa = rawVal;
    let upi = `upi://pay?pa=${pa}&pn=${pn}&mc=${mc}&tr=${tr}&tn=${tn}&cu=INR`;
    
    if (amount > 0) {
      upi += `&am=${reqAmount}&mam=${reqAmount}`;
    }
